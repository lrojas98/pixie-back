...
export class Controller {
    ...
    async getAll(req: any, res: Response, next: any) {
        ...
        try {
            const results = await Credit_cardsModel().findAll({
                where: {
                    users_id: token.user.id
                }
            });
            res.status(200).json({credit_cards: results});
        } catch (error) {
    ...
    async get(req: Request, res: Response, next: any) {
        try {
            ...
            const { id } = req.params;
            const reg = await Credit_cardsModel().findByPk(id);
    ...
    async add(req: Request, res: Response, next: any) {
        ...
        try {
            ...
            const reg = await Credit_cardsModel().create(body);
            res.json(reg);
