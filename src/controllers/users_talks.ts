...
export class Controller {
    ...
    async getAll(req: any, res: Response, next: any) {
        ...
        try {
            const results = await Users_talksModel().findAll({
                where: {
                    userId: token.id,
                    status: true
                },
                include: [{
                    model: Talks_messagesModel()
                }],
                order: [
                    ['createdAt', 'DESC'],
                ]
            });
            res.status(200).json({user_talks: results});
        } catch (error) {
    ...
    async get(req: Request, res: Response, next: any) {
        try {
            ...
            const { id } = req.params;
            const reg = await Users_talksModel().findByPk(id);
    ...
    async add(req: Request, res: Response, next: any) {
        ...
        try {
            ...
            const reg = await Users_talksModel().create(body);
            res.json(reg);
    ...
    static async SaveMessage(data:any){
        console.log(data)
    }
}
